  ...
  (bind ((*translation* (+ *translation* (location-of instance)))
    ...
    (if (typep elements 'computed-ll)
        (bind ((clipping (sdl:get-clip-rect))
               (count 0))
          (print-to-device (hu.dwim.computed-class::value-of elements) display)
          (rebind (*translation*)
            (iter (for element :initially (previous-element-of elements) :then (previous-element-of element))
                  (while (and element (> (+ (2d-y (+ *translation* (location-of (hu.dwim.computed-class::value-of element)))) 20) (sdl:y clipping))))
                  (incf count)
                  (print-to-device (hu.dwim.computed-class::value-of element) display)
                  (incf *translation* (location-of (hu.dwim.computed-class::value-of element)))))
          (rebind (*translation*)
            (iter (for element :initially (next-element-of elements) :then (next-element-of element))
                  (while (and element (< (2d-y (+ *translation* (location-of (hu.dwim.computed-class::value-of element)))) (sdl:y2 clipping))))
                  (incf count)
                  (print-to-device (hu.dwim.computed-class::value-of element) display)
                  (incf *translation* (location-of (hu.dwim.computed-class::value-of element))))))
