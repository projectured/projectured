...
(def function find-tree-node-reference (reference)
  (pattern-case reference
    ((the sequence-position (pos (the string (?or (elt (the list (children-of (the tree/node ?a))) ?b)
                                                  ((?or opening-delimiter closing-delimiter)
                                                   (the tree/node (elt (the list (children-of (the tree/node ?a))) ?b)) ?c)
                                                  (separator (the ?t1 (elt (the list (children-of (the tree/node ?a))) ?b))
                                                             (the ?t2 (elt (the list (children-of (the tree/node ?c))) ?d)) ?e))) ?f))
     `(the tree/node ,?a))))
  ...
  (bind ((latest-gesture (first (gestures-of gesture-queue))))
    (cond ((typep operation 'operation/sequence/replace-element-range)
          ...
          ((and (typep latest-gesture 'gesture/keyboard/key-press)
                (eq (key-of latest-gesture) :sdl-key-up)
                (member :sdl-key-mod-lalt (modifiers-of latest-gesture)))
