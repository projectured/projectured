...
(def printer inspector/object->table/table (projection recursion input input-reference)
  (bind ((instance (instance-of input))
         ...
         (type-label "TYPE")
         (type-label-text (text/make-text (list (text/make-string type-label :font *font/ubuntu/monospace/regular/18* :font-color *color/solarized/blue*))))
         (type-text (text/make-text (list (text/make-string (symbol-name class-name) :font *font/ubuntu/monospace/regular/18* :font-color *color/solarized/red*))))
         (output (if expanded?
...
(def printer inspector/object-slot->table/row (projection recursion input input-reference)
  (bind ((slot (slot-of input))
         ...
         (output (table/row ()
                   (table/cell ()
                     (text/make-text (list (text/make-string (symbol-name slot-name) :font *font/ubuntu/monospace/regular/18* :font-color *color/solarized/blue*))))
                   (table/cell ()
