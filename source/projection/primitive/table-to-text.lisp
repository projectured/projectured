                    ...
                    (prog1-bind elements nil
                      (flet ((write-character (character &optional (count 1))
                               (push (if (char= #\NewLine character)
                                         (text/newline :font *font/ubuntu/monospace/regular/24*)
                                         (text/make-string (make-string count :element-type 'character :initial-element character) :font *font/ubuntu/monospace/regular/24* :font-color *color/solarized/gray*)) elements)
                                                ...
                                                (text/concatenate (elt cell-text-lines text-line-index)
                                                                  ...
                                                                  (bind ((lines (text/split content #\NewLine)))
                                                                    (if (< text-line-index (length lines))
                                                                        (bind ((line (text/make-text (iter (for element :in-sequence (elements-of (elt lines text-line-index)))
                                                                                                           (collect (text/string (content-of element) :font *font/ubuntu/monospace/regular/24* :font-color (font-color-of element))))))
                                                                               (padding (text/make-text (list (text/make-string (make-string-of-spaces (max 0 (- column-width (text/length line)))) :font *font/ubuntu/monospace/regular/24* :font-color *color/default*)))))
                                                                          (text/concatenate line padding))
                                                                        (text/make-text (list (text/make-string (make-string-of-spaces column-width) :font *font/ubuntu/monospace/regular/24* :font-color *color/default*))))))))
                                    (finally
