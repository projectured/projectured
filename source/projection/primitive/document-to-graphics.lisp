...
(def printer document->graphics (projection recursion input input-reference output-reference)
  (bind ((typed-input-reference `(the ,(form-type input) ,input-reference))
         ...
         (iomap (recurse-printer recursion input-content `(content-of ,typed-input-reference) `(elt (the list (elements-of (the graphics/canvas ,output-reference))) 0)))
    ...
    (pattern-case input-selection
      ((?or (the character (elt (the string ?a) ?b))
       ...
       (bind ((graphics-reference nil))
         ...
         (bind ((selection-graphics (pattern-case graphics-reference
                ...
                (output (make-graphics/canvas (optional-list output-content selection-graphics) (make-2d 0 0))))
       ...
       (bind ((opening-delimiter-graphics-reference nil)
         ...
         (bind ((opening-delimiter-location (apply-reference iomap opening-delimiter-graphics-reference 'print))
           ...
           (make-iomap/recursive projection recursion input input-reference output output-reference
                                 (list (make-iomap/object projection recursion input input-reference output output-reference) iomap)))))
      (?a
       (make-iomap/object projection recursion input input-reference output-content output-reference)))))
