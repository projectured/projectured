...
(def printer word-wrapping (projection recursion input input-reference)
  ...
  (bind ((pack (as (iter (with x = 0)
                         ...
                         (when (> x wrap-width)
                           ...
                           (collect (make-text/string (string #\NewLine) :font *font/default* :font-color *color/default*) :into output-elements))
                         ...
                         (finally (return (list (elements-of (text/consolidate (make-text/text output-elements))) (reverse newline-insertion-indices)))))))
         ...
         (output (as (make-text/text (as (first (va pack))) :selection output-selection))))
