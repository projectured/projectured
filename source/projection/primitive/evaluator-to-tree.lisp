...
(def printer evaluator/evaluator->tree/node (projection recursion input input-reference)
  (bind ((result (if (on-demand-p input)
         ...
         (form-iomap (as (recurse-printer recursion (form-of input) `((form-of (the evaluator/evaluator document))
                                                                      ,@(typed-reference (form-type input) input-reference)))))
         (result-iomap (as (recurse-printer recursion result `((result-of (the evaluator/evaluator document))
                                                               ,@(typed-reference (form-type input) input-reference)))))
         (output-selection (as (pattern-case (reverse (selection-of input))
                                 (((the ?value-type (form-of (the evaluator/evaluator document)))
                                   . ?rest)
                                  (append (selection-of (output-of (va form-iomap)))
