    ...
    (if (eq child-operation operation)
        ...
        (labels ((recurse (child-operation)
                   (cond ((typep child-operation 'operation/compound)
                          (make-operation/compound (mapcar #'recurse (elements-of child-operation))))
                          ...
                          (bind ((domain-reference nil))
                            (map-backward projection-iomap (tree-replace (target-of child-operation) '(the document document) `(the document ,(output-reference-of projection-iomap)))
                                          (lambda (iomap reference)
                                            ...
                                            (setf domain-reference reference)))
                            (if domain-reference
                                (make-operation/sequence/replace-element-range input (tree-replace domain-reference `(the document ,(input-reference-of projection-iomap)) '(the document document)) (replacement-of child-operation))
                                child-operation)))
                          ...
                          (bind ((domain-reference nil))
                            (map-backward projection-iomap (tree-replace (target-of child-operation) '(the document document) `(the document ,(output-reference-of projection-iomap)))
                                          (lambda (iomap reference)
                                            ...
                                            (setf domain-reference reference)))
                            (if domain-reference
                                (make-operation/number/replace-range input (tree-replace domain-reference `(the document ,(input-reference-of projection-iomap)) '(the document document)) (replacement-of child-operation))
                                child-operation)))
                         (t child-operation))))
          (recurse child-operation))
        child-operation)))
