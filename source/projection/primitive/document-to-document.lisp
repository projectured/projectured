...
(def function operation/read-backward (operation printer-iomap)
  (labels ((recurse (child-operation)
             (cond ((typep child-operation 'operation/quit)
                    child-operation)
                   ((typep child-operation 'operation/save-document)
                    (make-instance 'operation/save-document :document (input-of printer-iomap) :filename (filename-of child-operation)))
                   ((typep child-operation 'operation/load-document)
                    (make-instance 'operation/load-document :document (input-of printer-iomap) :filename (filename-of child-operation)))
                   ((typep child-operation 'operation/export-document)
                    (make-instance 'operation/export-document :document (input-of printer-iomap) :filename (filename-of child-operation)))
                   ((typep child-operation 'operation/import-document)
                    (make-instance 'operation/import-document :document (input-of printer-iomap) :filename (filename-of child-operation)))
                   #+nil
                   ((typep child-operation 'operation/replace-selection)
                    (awhen (project-backward printer-iomap (selection-of child-operation))
                      (make-operation/replace-selection (input-of printer-iomap) it)))
                   #+nil
                   ((typep child-operation 'operation/sequence/replace-element-range)
                    (awhen (project-backward printer-iomap (target-of child-operation))
                      (make-operation/sequence/replace-element-range (input-of printer-iomap) it (replacement-of child-operation))))
                   #+nil
                   ((typep child-operation 'operation/number/replace-range)
                    (awhen (project-backward printer-iomap (target-of child-operation))
                      (make-operation/number/replace-range (input-of printer-iomap) it (replacement-of child-operation))))
