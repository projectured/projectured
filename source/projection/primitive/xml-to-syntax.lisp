  ...
  (bind ((operation-mapper (lambda (operation selection child-selection child-iomap)
    ...
    (merge-commands (gesture-case -gesture-
                      ((make-type-in-gesture #\")
                       ...
                       :operation (make-operation/replace-target nil (xml/text (:selection '((the string (value-of (the xml/text document)))
                                                                                                             (the string (subseq (the string document) 0 0))))
                                                                                       "")))
                      ((make-type-in-gesture #\<)
                       :domain "XML" :description "Inserts a new XML element into the children of the XML element"
                       :operation (make-operation/replace-target nil (xml/element ("" nil :selection `((the string (xml/start-tag (the xml/element document)))
                                                                                                                       (the string (subseq (the string document) 0 0))))))))
                               ...
                               (operation/text/replace-range
                                (reference-case selection
                                  ...
                                  (((the syntax/leaf (printer-output (the xml/text document) ?projection ?recursion)) . ?rest)
                                   (make-operation/string/replace-range '((the string (value-of (the xml/text document)))
                                                                          (the string (subseq (the string document) 0 0)))
                               ...
                               (operation/text/replace-range
                                (reference-case selection
                                  ...
                                  (((the syntax/node (printer-output (the xml/attribute document) ?projection ?recursion))
                                    ...
                                    (the syntax/leaf (elt (the sequence document) 0))
                                    . ?rest)
                                   (make-operation/string/replace-range '((the string (name-of (the xml/attribute document)))
                                                                          (the string (subseq (the string document) 0 0)))
                                                                        (replacement-of operation)))
                                  (((the syntax/node (printer-output (the xml/attribute document) ?projection ?recursion))
                                    (the sequence (children-of (the syntax/node document)))
                                    (the syntax/leaf (elt (the sequence document) 1))
                                    . ?rest)
                                   (make-operation/string/replace-range '((the string (value-of (the xml/attribute document)))
                                                                          (the string (subseq (the string document) 0 0)))
                                                                        (replacement-of operation)))))))))
    (merge-commands (gesture-case -gesture-
                      ((make-type-in-gesture #\=)
                       ...
                       :operation (reference-case (selection-of -printer-input-)
                                    (((the string (name-of (the xml/attribute document)))
                                      (the string (subseq (the string document) ?start-character-index ?end-character-index)))
                                     (make-operation/replace-selection '((the string (value-of (the xml/attribute document)))
                                                                         (the string (subseq (the string document) 0 0))))))))
                    (command/read-backward -recursion- -input- -printer-iomap- operation-mapper)
                    (make-nothing-command -gesture-))))
                               ...
                               (operation/text/replace-range
                                (reference-case selection
                                  ...
                                  (((the syntax/node (printer-output (the xml/element document) ?projection ?recursion))
                                    ...
                                    (the syntax/leaf (elt (the sequence document) 0))
                                    . ?rest)
                                   (make-operation/string/replace-range '((the string (xml/start-tag (the xml/element document)))
                                                                          (the string (subseq (the string document) 0 0)))
                                                                        (replacement-of operation)))
                                  (((the syntax/node (printer-output (the xml/element document) ?projection ?recursion))
                                    (the sequence (children-of (the syntax/node document)))
                                   ...
                                   (when (= ?child-index (1- (length (children-of (output-of -printer-iomap-)))))
                                     (make-operation/string/replace-range '((the string (xml/end-tag (the xml/element document)))
                                                                            (the string (subseq (the string document) 0 0)))
                                                                          (replacement-of operation))))))))))
                    ...
                    (gesture-case -gesture-
                      ((make-key-press-gesture :scancode-insert)
                       ...
                       :operation (make-operation/compound (bind ((children-length (length (children-of -printer-input-))))
                                                             (list (make-operation/sequence/replace-range `((the sequence (children-of (the xml/element document)))
                                                                   ...
                                                                   (make-operation/replace-selection `((the sequence (children-of (the xml/element document)))
                       ...
                       :operation (reference-case (selection-of -printer-input-)
                                    ((?or ((the string (xml/start-tag (the xml/element document)))
                                     ...
                                     (bind ((index (length (attributes-of -printer-input-))))
                                       (make-operation/compound (list (make-operation/sequence/replace-range `((the sequence (attributes-of (the xml/element document)))
                                                                                                               (the sequence (subseq (the sequence document) ,index ,index)))
                                                                                                             (list (xml/attribute () "" "")))
                                                                      (make-operation/replace-selection `((the sequence (attributes-of (the xml/element document)))
                      ...
                      ((make-type-in-gesture #\")
                       ...
                       :operation (bind ((index (length (children-of -printer-input-))))
                                    (make-operation/compound (list (make-operation/sequence/replace-range `((the sequence (children-of (the xml/element document)))
                                                                                                            (the sequence (subseq (the sequence document) ,index ,index)))
                                                                                                          (list (xml/text () "")))
                                                                   (make-operation/replace-selection `((the sequence (children-of (the xml/element document)))
