...
(def printer workbench/workbench->widget/shell ()
  (bind ((navigator-iomap (recurse-printer -recursion- (navigator-of -input-)
                                           `((navigator-of (the workbench/workbench document))
                                             ,@(typed-reference (document-type -input-) -input-reference-))))
         (console-iomap (recurse-printer -recursion- (console-of -input-)
                                         `((console-of (the workbench/workbench document))
                                           ,@(typed-reference (document-type -input-) -input-reference-))))
         (editor-iomap (recurse-printer -recursion- (editor-of -input-)
                                        `((editor-of (the workbench/workbench document))
                                          ,@(typed-reference (document-type -input-) -input-reference-))))
...
(def printer workbench/navigator->widget/title-pane ()
  (bind ((folder-iomaps (iter (for index :from 0)
                              ...
                              (collect (recurse-printer -recursion- folder
                                                        `((elt (the sequence document) ,index)
                                                          ...
                                                          ,@(typed-reference (document-type -input-) -input-reference-))))))
         (output (widget/title-pane (:title (text/text () (text/string "Navigator" :font *font/ubuntu/regular/24* :font-color *color/black*))
                                     :title-fill-color *color/solarized/content/lighter*
                                     :border (make-inset :all 5)
                                     :border-color *color/solarized/content/lighter*)
                   (widget/scroll-pane (:size (make-2d 224 655)
...
(def printer workbench/console->widget/title-pane ()
  (bind ((content-iomap (recurse-printer -recursion- (content-of -input-)
                                         `((content-of (the workbench/document document))
                                           ,@(typed-reference (document-type -input-) -input-reference-))))
         (output (widget/title-pane (:title (text/text () (text/string "Console" :font *font/ubuntu/regular/24* :font-color *color/black*))
                                     :title-fill-color *color/solarized/content/lighter*
                                     :border (make-inset :all 5)
                                     :border-color *color/solarized/content/lighter*)
                   (widget/scroll-pane (:size (make-2d 1000 130)
                                        :padding (make-inset :all 5)
                                        :padding-color *color/white*)
                     (output-of content-iomap)))))
    (make-iomap/compound -projection- -recursion- -input- -input-reference- output (list content-iomap))))

(def printer workbench/editor->widget/tabbed-pane ()
  (bind ((document-iomaps (iter (for index :from 0)
         ...
         (output (make-widget/tabbed-pane (iter (for document :in-sequence (documents-of -input-))
                                                ...
                                                (collect (list (text/text () (text/string (title-of document) :font *font/ubuntu/regular/24* :font-color *color/black*))
...
(def reader workbench/navigator->widget/title-pane ()
  (recurse-reader -recursion- -input- (elt (child-iomaps-of -printer-iomap-) 0)))

(def reader workbench/console->widget/title-pane ()
  (recurse-reader -recursion- -input- (elt (child-iomaps-of -printer-iomap-) 0)))

(def reader workbench/editor->widget/tabbed-pane ()
