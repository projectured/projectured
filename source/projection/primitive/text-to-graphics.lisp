...
(def printer text->graphics (projection recursion input input-reference)
  ...
  (bind ((graphics-element-indices nil)
         ...
         (line-heights (iter outer
                             (text/map-split input #\NewLine
                                             (lambda (start-element-index start-character-index end-element-index end-character-index)
                                               ...
                                               (in outer (collect (or (iter (for index :from start-element-index :below end-element-index)
         ...
         ((:values preceding-selection-elements following-selection-elements)
          (labels ((graphics-character-index (text-character-index)
                     (iter (for index :from 0)
                           (for first-character-index :in (reverse first-character-indices))
                           (for last-character-index :in (reverse last-character-indices))
                 ...
                 (values nil
                         (list (make-graphics/line (+ location (make-2d offset 0))
                               ...
                               (make-graphics/line (+ location (make-2d (- offset 2) height))
                                                   ...
                                                   :stroke-color *color/black*)))))
              (((the text/text (text/subseq (the text/text document) ?text-start-character-index ?text-end-character-index)))
               (bind (((:values graphics-element-index graphics-character-index) (graphics-character-index ?text-start-character-index))
                      (text-graphics (elt elements graphics-element-index))
                 ...
                 (values nil (list (make-graphics/rectangle (+ location (make-2d offset 0)) (measure-text text font) :fill-color *color/solarized/background/light*)))))
    ...
    (make-iomap 'iomap/text->graphics
                ...
                :first-character-indices (nreverse first-character-indices)
                :last-character-indices (nreverse last-character-indices))))
