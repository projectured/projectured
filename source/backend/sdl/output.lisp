...
(def function output-to-renderer (backend renderer instance translation)
  (typecase instance
    ...
    (graphics/polygon
     (cffi:with-foreign-objects ((xs :int16 (length (points-of instance)))
                                 (ys :int16 (length (points-of instance))))
       (iter (for i :from 0)
             (for point :in-sequence (points-of instance))
             (incf point translation)
             (setf (cffi:mem-ref xs :int16 i) (round (2d-x point)))
             (setf (cffi:mem-ref ys :int16 i) (round (2d-y point))))
