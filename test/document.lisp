...
(def function make-test-document/clipboard (content)
  (document/clipboard ()
    content))
...
(def function make-test-document/scroll (content)
  (widget/shell (:size (make-2d 1280 720))
    (widget/scroll-pane (:location 0 :size (make-2d 1280 720) :margin (make-inset :all 5))
      content)))

(def function make-test-document/search (content &optional search-string)
  (bind ((document
          (document/search (:search search-string)
            content)))
    (widget/shell ()
      (widget/composite (:location 0 :selection '((the sequence (elements-of (the widget/composite document)))
                                                              (the widget/scroll-pane (elt (the sequence document) 0))
                                                              (the document/search (content-of (the widget/scroll-pane document)))))
        (widget/scroll-pane (:location 0 :size (make-2d 1280 690) :margin (make-inset :all 5))
          document)
        (widget/composite (:location (make-2d 988 0))
          document)))))

(def function make-test-document/split (content-1 content-2)
  (widget/shell ()
    (widget/split-pane ()
      (widget/scroll-pane (:location 0 :size (make-2d 640 720) :margin (make-inset :all 5))
        content-1)
      (widget/scroll-pane (:location 0 :size (make-2d 640 720) :margin (make-inset :all 5))
        content-2))))
...
(def document test/selection ()
  ((content :type t)))

(def macro test/selection (() &body content)
  `(make-instance 'test/selection :content ,(first content)))
...
(def document test/gesture ()
  ((content :type t)
   (last-commands :type sequence)))

(def macro test/gesture (() &body content)
  `(make-instance 'test/gesture :content ,(first content) :last-commands nil))

(def function make-test-document/gesture (content)
  (test/gesture ()
    content))
...
(def function make-test-document/graphics/ll (element-count origin-index)
  (make-graphics/canvas (elt-ll (ll (iter (for i :from 0 :below element-count)
                                          (collect (make-graphics/canvas (list (make-graphics/text 0 "Hello " :font *font/default* :font-color *color/default* :fill-color nil)
...
(def function make-test-document/json/number ()
  (json/number () (document/number () 42)))
...
(def function make-test-document/json/object ()
  (json/object ()
    ...
    ("true" (json/boolean () #t))
    ("number" (json/number () (document/number () 42)))
...
(def function make-test-document/json ()
  (json/array ()
    ...
    (json/number () (document/number () 42))
    (json/string () "Hello World")
    (json/object (:key 'key-of :predicate 'string<)
      ("null" (json/null ()))
      ("false" (json/boolean () #f))
      ("true" (json/boolean () #t))
      ("number" (json/number () (document/number () 42)))
    ...
    (setf (body-of factorial-function)
          (list (make-common-lisp/if (make-common-lisp/application (make-lisp-form/symbol* '<)
                                                                   (list (make-common-lisp/variable-reference factorial-argument)
                                                                         (make-common-lisp/constant (document/number () (if faulty 3 2)))))
                                     (make-common-lisp/constant (document/number () 1))
...
(def function make-test-document/t/sequence ()
  (document/sequence ()
    ...
    (make-instance 'test-object :test-instance-slot (document/sequence () "Hello World"))))
  ...
  (bind ((json-document (make-test-document/json))
    ...
    (book/book (:title "ProjecturEd, the Projectional Editor" :author "Levente Mészáros")
      (book/chapter (:title "Domains")
        #+nil
        (book/chapter (:title "Graphics" :collapsed #t)
          (book/paragraph ()
            (text/text ()
        ...
        #+nil
        (book/chapter (:title "List" :collapsed #t)
          (book/paragraph ()
            (text/text ()
              (text/string "Some list")))
          (make-test-document/list))
        #+nil
        (book/chapter (:title "Tree" :collapsed #t)
          (book/paragraph ()
            (text/text ()
      ...
      (book/chapter (:title "Projections")
        (book/chapter (:title "Sorting" :collapsed #t)
          (book/paragraph ()
            (text/text ()
          ...
          json-document)
        (book/chapter (:title "Filtering" :collapsed #t)
          (book/paragraph ()
            (text/text ()
          ...
          json-document)
        (book/chapter (:title "Selecting" :collapsed #t)
          (book/paragraph ()
            (text/text ()
