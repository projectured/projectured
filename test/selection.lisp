...
(def function make-test-selection/table/character/nested ()
  '((the sequence-position (text/pos (the text/text document) 1))
    ...
    (the list (cells-of (the table/row document)))
    (the table/row (elt (the list document) 1))
    (the list (rows-of (the table/table document)))
...
(def function make-test-selection/table/cell ()
  '(the table/cell (elt (the list (cells-of (the table/row (elt (the list (rows-of (the table/table document))) 1)))) 0)))
...
(def function make-test-selection/tree/character ()
  '((the sequence-position (text/pos (the text/text document) 1))
    ...
    (the tree/node (elt (the list document) 1))
    (the list (children-of (the tree/node document)))))
...
(def function make-test-selection/json/object ()
  '(the json/object (elt (the list (elements-of (the json/array document))) 5)))

(def function make-test-selection/json/character ()
  '(the character (elt (the string (value-of (the json/string (elt (the list (elements-of (the json/array document))) 4)))) 8)))
...
(def function make-test-selection/json/character-position ()
  '((the sequence-position (pos (the string document) 8))
    ...
    (the json/string (elt (the list document) 4))
...
(def test test/selection/navigation (content selection projection)
  (iter (with executed-operation-count = 0)
        ...
        (iter (for command :in-sequence (operations-of command-map))
